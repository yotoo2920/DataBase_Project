...
def query5(request):
	with connection.cursor() as cursor:
		cursor.execute("SELECT COMPANY, AVG_WEEKLY_SAL*12 AS AVG_SALARY FROM(SELECT DISTINCT ORGANIZATION.NAME AS COMPANY, AVG(PERSON.WEEKLY_SALARY) AS AVG_WEEKLY_SAL FROM ORGANIZATION JOIN JOBS ON ORGANIZATION.NAME = JOBS.COMPANY_NAME JOIN PERSON ON JOBS.NAME = PERSON.OCCUPATION GROUP BY ORGANIZATION.NAME ORDER BY AVG_WEEKLY_SAL DESC) WHERE ROWNUM < 11")
		rows = dictfetchall(cursor)
